...
from Preprocess import ROWS, COLS, TRAIN_DIR, TRAINING_SIZE, VALIDATION_SIZE, read_test_data, FEATURES_TEST_NPY, \
    FEATURES_VALIDATION_NPY, FEATURES_TRAIN_NPY, BATCH_SIZE
...
def get_trained_validation_features():
    ...
    validation_labels = np.array([0] * (VALIDATION_SIZE / 2) + [1] * (VALIDATION_SIZE / 2))
    ...
    def create_bottleneck_features(self, imageGenerator):
        if not os.path.isfile(FEATURES_TRAIN_NPY):
            generator = imageGenerator.flow_from_directory(
                ...
                class_mode=None,
                shuffle=False)
            bottleneck_features_train = self._model.predict_generator(generator, TRAINING_SIZE)
        ...
        if not os.path.isfile(FEATURES_VALIDATION_NPY):
            generator = imageGenerator.flow_from_directory(
                ...
                class_mode=None,
                shuffle=False)
            bottleneck_features_validation = self._model.predict_generator(generator, VALIDATION_SIZE)

...
class ConvolutionNet:
    ...
    def create_network(self):
        ...
        up6 = merge(
            [UpSampling2D(size=(2, 2))(conv5), conv4], mode="concat", concat_axis=1
        )
        conv6 = Convolution2D(256, 3, 3, activation="relu", border_mode="same")(up6)
        conv6 = Convolution2D(256, 3, 3, activation="relu", border_mode="same")(conv6)

        up7 = merge(
            [UpSampling2D(size=(2, 2))(conv6), conv3], mode="concat", concat_axis=1
        )
        conv7 = Convolution2D(128, 3, 3, activation="relu", border_mode="same")(up7)
        conv7 = Convolution2D(128, 3, 3, activation="relu", border_mode="same")(conv7)

        up8 = merge(
            [UpSampling2D(size=(2, 2))(conv7), conv2], mode="concat", concat_axis=1
        )
        conv8 = Convolution2D(64, 3, 3, activation="relu", border_mode="same")(up8)
        conv8 = Convolution2D(64, 3, 3, activation="relu", border_mode="same")(conv8)

        up9 = merge(
            [UpSampling2D(size=(2, 2))(conv8), conv1], mode="concat", concat_axis=1
        )
